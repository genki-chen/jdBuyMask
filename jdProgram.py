...
from util import parse_json, response_status, split_area_id
import traceback
...
def check_new(session, skuids, area):
    """获取多个商品库存状态（旧）
...
#    start = int(time.time() * 1000)
    ...
    resp = session.post(url=url, headers=headers, data=data, timeout=5)
    # print(resp.text)
...
#    logger.info('检测[%s]个口罩有货，[%s]个口罩无货，耗时[%s]ms', len(inStockSkuid), len(nohasSkuid),
#                int(time.time() * 1000) - start)
    return inStockSkuid
    ...
    for count in range(1, 3):
        ...
        try:
            if is_Submit_captcha:
                captcha_result = page_detail_captcha(session, encryptClientInfo, captchaUrl)
                # 验证码服务错误
            ...
            if resp_json.get('success'):
                ...
                if result_code == 0:
                    ...
                    resultMessage = resultMessage + '(无货了)'
...
def page_detail_captcha(session, isId, captchaUrl):
    ...
    try:
        ...
        result = analysis_captcha(session, captchaUrl, resp.content)
        if not result:
